...
| 파일 업로드 | TXT 파일을 버튼 클릭 또는 드래그&드롭으로 바로 입력 |
...
| 면접 예상 질문 | 자기소개서 기반으로 예상 면접 질문 10개와 답변 팁 생성 |
| 다크 모드 | 화면 우상단 버튼으로 라이트/다크 테마 전환, 설정 자동 저장 |
...
- `Content-Type: text/event-stream`
- 스트리밍 방식으로 질문 목록 전송
- 기본 질문 3개 / 심화 질문 4개 / 압박·돌발 질문 3개 + 각 질문별 답변 팁
